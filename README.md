...
### 🛡️ 站点适配与防爬处理
- 🔒 **Cloudflare 验证与状态感知**：内置验证窗口以获取和管理 Cloudflare 凭证，能自动清除失效凭证并提示重新验证。
...
### 🎨 界面与交互
- 🪶 **极致微小与低耗资源 (Ultra Lightweight)**：得益于 Tauri v2 原生架构，安装包与二进制仅约 **10 MB+** 级别，内存占用相比传统 Electron 降低超过 **80%**，毫秒级极速启动。
...
- 🎨 **扁平化设计与多主题支持**：采用 shadcn/ui 扁平化视觉规范，内置深色模式与浅色模式，支持一键实时无缝切换。
