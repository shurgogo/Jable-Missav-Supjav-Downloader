...
### 🛡️ 站點適配與防爬處理
- 🔒 **Cloudflare 驗證與狀態感知**：內建驗證視窗以獲取和管理 Cloudflare 憑證，能自動清除失效憑證並提示重新驗證。
...
### 🎨 介面與互動
- 🪶 **極致微小與低耗資源 (Ultra Lightweight)**：得益於 Tauri v2 原生架構，安裝包與二進位檔僅約 **10 MB+** 級別，記憶體佔用相比傳統 Electron 降低超過 **80%**，毫秒級極速啟動。
...
- 🎨 **扁平化設計與多主題支援**：採用 shadcn/ui 扁平化視覺規範，內建深色模式與淺色模式，支援一鍵即時無縫切換。
