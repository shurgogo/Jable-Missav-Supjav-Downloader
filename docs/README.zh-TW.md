...
基於 Tauri v2、React 與 Rust 構建的高性能桌面影片下載與瀏覽工具，支援 JableTV、MissAV 與 SupJav。
...
- 👁️ **卡片懸停預覽**：懸停卡片時載入影片預覽片段並產生記憶體 Blob URL 播放。
- 📋 **批量任務控制與雙區管理**：下載佇列分為「進行中」與「已完成」兩大區塊，提供雙行緊湊佈局與 Hover 工具提示，支援全選、多選以及批量開始、暫停、取消和一鍵清空紀錄。
- 🎨 **多主題切換支援**：內建 daisyUI 多套主題外觀（包含深色模式與淺色模式），支援一鍵即時無縫切換。
